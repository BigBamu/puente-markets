...
function isLikelyFootball(event, market) {
  ...
  const text = [event?.title,event?.subtitle,event?.category,event?.subcategory,market?.question,
    market?.marketType,market?.sportsMarketType,...series.flatMap(s=>[s?.title,s?.slug,s?.description]),
    ...tags.flatMap(t=>[t?.label,t?.slug])].filter(Boolean).join(" ").toLowerCase();
  ...
  return Boolean((market?.gameStartTime || event?.gameStartTime) && (/\b(vs\.?|v\.|versus)\b/.test(title) || title.includes(" - ")));
}
function normalizeMarket(market, event = null) {
  const outcomes = parseArray(market.outcomes);
  const prices = parseArray(market.outcomePrices).map(numeric);
  ...
  return {
    ...
    slug: market.slug || "", eventTitle: eventObj?.title || market.question || "Polymarket",
    eventSlug: eventObj?.slug || "", image: market.icon || market.image || eventObj?.icon || eventObj?.image || "",
    category: market.sportsMarketType || market.marketType || market.formatType || "Övrigt",
    outcomes: outcomes.length ? outcomes : ["Yes","No"],
    yesPrice: prices[yesIndex] ?? prices[0] ?? 0.5, noPrice: prices[noIndex] ?? prices[1] ?? 0.5,
    yesTokenId: tokens[yesIndex] || tokens[0] || "", noTokenId: tokens[noIndex] || tokens[1] || "",
    volume: numeric(market.volumeNum ?? market.volume), volume24h: numeric(market.volume24hr),
    ...
    endTime: market.endDate || eventObj?.endDate || "", acceptingOrders: market.acceptingOrders !== false,
    isFootball: isLikelyFootball(eventObj, market),
    url: eventObj?.slug ? `https://polymarket.com/event/${eventObj.slug}` : market.slug ? `https://polymarket.com/market/${market.slug}` : "https://polymarket.com"
  };
}
async function getJson(url) {
  ...
  try {
    const response = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "Puente-Markets/0.2" }, signal: controller.signal });
    ...
    try {
      ...
      for (const event of Array.isArray(events) ? events : []) {
        for (const market of Array.isArray(event.markets) ? event.markets : []) {
          ...
          const item = normalizeMarket(market,event); if (item.isFootball) normalized.push(item);
        }
      }
      if (normalized.length < 5) {
        const direct = await getJson(`${GAMMA}/markets?active=true&closed=false&limit=${limit}`);
        for (const market of Array.isArray(direct) ? direct : []) {
          const item = normalizeMarket(market); if (item.isFootball && !normalized.some(m=>m.id===item.id)) normalized.push(item);
        }
      }
      normalized.sort((a,b)=>(b.liquidity+b.volume24h)-(a.liquidity+a.volume24h));
      return Response.json({ source:"polymarket", fetchedAt:new Date().toISOString(), count:normalized.length, markets:normalized.slice(0,limit) },
        { headers:{ "Cache-Control":"s-maxage=20, stale-while-revalidate=60" } });
    } catch (error) {
      return Response.json({ source:"error", fetchedAt:new Date().toISOString(), error:error instanceof Error?error.message:"Kunde inte hämta marknader", markets:[] }, { status:502 });
    }
  }
};
